...
public abstract class DifficultyRecipe<T extends IRecipe> implements IDifficultyRecipe<T> {
    ...
    public boolean matches(InventoryCrafting inv, World world) {
        if (world == null) {
            ...
            if (!pastStacks.contains(stack)) {
                FMLLog.bigWarning(
                    "A mod has attempted to check a recipe (%s) without world context. This is a bug and should be reported to the mod author.",
                    cur.getRecipeOutput());
                LogManager.getLogger()
                    .info("Continuing with default difficulty for recipe");
    ...
    public static <T extends IRecipe> Collection<Difficulty> getDuplicatedRecipes(DifficultyRecipe<T> recipe,
        Difficulty actual) {
